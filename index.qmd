...
|---|---------------|----:|
| `y` |  The noisy time-series to be differentitated to the order `d`. This should be a `numpy` array. Notice that this sequence should be normalized so that the maximum absolute value is lower that 1.0 (approximately outliers excluded)| --|
